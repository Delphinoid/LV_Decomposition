...
function solution(C, J)
	...
	_, I := Dimension(J);
	// Potential issue: what if J captures multiple
	// primitive idempotents? I wrote code to address
	// this, please see the procedure commented below.
	// I've never run into a situation where it has
	// been necessary, so I'd rather not include it.
	return VarietySequence(J + ideal<C | [C.i : i in I]>)[1];
end function;

//procedure solutions(M, C, CR, J, ~idempotents)
//
//	// The Krull dimension of the ideal is the maximal cardinality of
//	// the set of indeterminates in C such that there is no leading
//	// monomial depending only on variables in C. The indeterminates
//	// not in this set are our independent variables. Occasionally,
//	// the ideal J may capture multiple primitive ideals. This happens
//	// (for instance) on the final layer of SU(3, 1) when we choose
//	// not to sort the graded bases.
//	X := [C.i : i in [1..Ngens(C)]];
//	Y := [];
//	G := GroebnerBasis(J);
//	i := 1; j := 1;
//	for i := 1 to Ngens(C) do
//		// We assume that the leading monomials are degree 1,
//		// as otherwise they cannot give us valid solutions.
//		// Thus the independent variables will be exactly
//		// those not appearing as leading monomials, and they
//		// will be the only terms appearing after leading terms.
//		// Dimension can give us a set I of independent variables,
//		// but it might not be compatible with the Groebner basis.
//		if j le #G and LeadingMonomial(G[j]) eq C.i then
//			X[i] -:= G[j];
//			j +:= 1;
//		else
//			Append(~Y, i);
...
//	end for;
//	
//	V := VarietySequence(J + ideal<C | [C.i : i in Y]>)[1];
//	E := ChangeRing(M, R,
//		hom<CR -> R |
//			hom<C -> Rationals() | V>,
//			[R.i : i in [1..Ngens(R)]]
...
//	if #Y gt 0 then
//	
//		B := PolynomialRing(Rationals(), #Y);
//		BR := PolynomialRing(C, Ngens(R));
//		
//		CB := [B!0 : i in [1..Ngens(C)]];
//		for i := 1 to #Y do
//			CB[Y[i]] := B.i;
//		end for;
//		M := ChangeRing(M, CR,
...
//		S := Eltseq(E*M);
//
//		// Currently, the expressions in S live inside BR.
...
//		// reduce our system to a system of equations in B.
//		i := 1;
//		while i le #S do
...
//				E := ChangeRing(M, R,
//					hom<BR -> R |
//						hom<B -> Rationals() | VarietySequence(J)[1]>,
//						[R.i : i in [1..Ngens(R)]]
//					>
//				);
//				Append(~idempotents,
...
procedure decompose(basis, action, ~primitives)
	...
	// Occasionally, multiple ideals in D may capture
	// the same primitive ideals. This happens (for
...
function isomorphic(new_basis, new_action, basis, action)
	...
	if SAFE then
		...
		for J in D do
			...
			// Check if the determinant (C.num_coeff) is non-zero by checking
			// whether it's the final element of the Groebner basis.
			G := GroebnerBasis(J);
			if IsEmpty(G) or G[#G] ne C.num_coeff then
				return true, d;
			end if;
		end for;
	else
		D := RadicalDecomposition(I);
		for J in D do
			// We expect Hom^0(Q, Q') to be the ground field if Q is
			// isomorphic to Q' and 0 otherwise, as an analogue to the
			// "almost-orthogonality" of the KL basis.
			G := GroebnerBasis(J);
			if G ne groebner_zero then
				return true, d;
			end if;
		end for;
	end if;
